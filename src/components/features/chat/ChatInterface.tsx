...
import { ModalContainerBox } from "@/components/modal-container-box"
...
export function ChatInterface({ onSlashCommand, currentChatId, onChatChange }: ChatInterfaceProps) {
  ...
  const [openModals, setOpenModals] = useState<{ type: string; id: string; zIndex: number; isCollapsed?: boolean; position?: { x: number; y: number } }[]>([])
  ...
  const [aiModel, setAiModel] = useState<{ provider: string | null; model: string | null; displayName: string } | null>(null)
  const messagesEndRef = useRef<HTMLDivElement>(null)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const { createChat, updateChat, getChat } = useChatStorage()

  // Handle position changes from modals
  const handlePositionChange = useCallback((modalId: string, x: number, y: number, isCollapsed: boolean) => {
    setOpenModals(prev => prev.map(m => 
      m.id === modalId 
        ? { ...m, position: { x, y }, isCollapsed }
        : m
    ))
  }, [])
  
  // Handle modal click from container (expand modal)
  const handleModalClick = useCallback((modalId: string) => {
    const modal = openModals.find(m => m.id === modalId)
    if (modal && modal.isCollapsed) {
      // Trigger expansion by updating the modal state
      // The CookForm component will handle the actual expansion
      setOpenModals(prev => prev.map(m => 
        m.id === modalId 
          ? { ...m, isCollapsed: false, position: { x: 0, y: 0 } }
          : m
      ))
    }
  }, [openModals])
  
  // Handle modal close from container
  ...
  const handleSlashCommandSelect = (command: string) => {
    ...
    if (command === "/cook") {
      const modalId = `cook-${Date.now()}`
      const newZIndex = 50 + openModals.length * 10
      setOpenModals([...openModals, { type: 'cook', id: modalId, zIndex: newZIndex }])
    } else {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white h-full">
      ...
      {/* Modal Container Box - wraps modals */}
      <ModalContainerBox 
        modals={openModals}
        zIndex={60}
      ...
      >
        {/* Cook Form Modals - Multiple instances */}
        {openModals
          .filter(m => m.type === 'cook')
          .map((modal, index) => {
            // Create modal-specific callback wrapper
            const modalPositionChange = (x: number, y: number, isCollapsed: boolean) => {
              handlePositionChange(modal.id, x, y, isCollapsed)
            }
            
            // Calculate stack index for collapsed modals
            const collapsedModals = openModals.filter(m => m.type === 'cook' && m.isCollapsed)
            const stackIndex = collapsedModals.findIndex(m => m.id === modal.id)
            ...
            return (
              <CookForm
                ...
                isOpen={true}
                onClose={() => {
                  setOpenModals(prev => prev.filter(m => m.id !== modal.id))
                }}
                onSubmit={(data) => {
                ...
                zIndex={modal.zIndex}
                onPositionChange={modalPositionChange}
                externalCollapsed={modal.isCollapsed}
